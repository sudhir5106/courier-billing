	 ...
	 $html='
			<html>
				...
				<table width="500" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td width="80"><img width="20%" src="../../image_upload/franchise/thumb/'.$cust[1]['Franchise_Logo'].'" alt=""></td>
				...
				<table width="500px" border="0" cellpadding="0" cellspacing="0">
				 <tr>
						<td colspan="2" valign="top">
							<h1 style="text-transform: uppercase; margin-bottom:0; font-size:14pt; font-weight:bold;">'.$cust[1]['Branch_Name'].'</h1>
							(Franchise of - '.$cust[1]['Franchise_Name'].')<br>'.$cust[1]['Address'].' '.$cust[1]['City'].', India
						</td>
						
					 </tr>
				 <tr><td colspan="2"><hr></td></tr>
				 <tr>
					   <td colspan="2"><strong>PAN No.</strong>:- '.$cust[1]['PAN_No'].', <strong>Service Tax No</strong>:- '.$cust[1]['Service_Tax_No'].', <strong>Category</strong>:- Courier Service </td>
				 </tr>
				 ...
				 <tr>			
					<td>
						...
						<br/>
						'.$cust[1]['Client_Name'].'
						<br/>'.$cust[1]['Client_Address'].', India 
					</td>
					<td>
				...
				<table style="font-size:10pt; border-spacing: 0 0;" width="500px" border="1">
						...
						foreach($cust as $custVal){						
							 
							 $bookingDate=$custVal['Date'];
							 $ConsignmentNo=$custVal['Consignment_No'];
							 $Destination=$custVal['Destination_Name'];
							 $Weight=$custVal['Total_Weight_In_KG'];
							 $Amount=sprintf('%0.2f',$custVal['Total_Amount']);
							 $insuChrg=sprintf('%0.2f',$custVal['Insurance_Other_Charges']);
							 $FnalAmount=sprintf('%0.2f',$custVal['FinalAmount']);
							 
							
							 $row.='
							  <tr>							
								<td width="30">'.$i.'</td>
								<td width="50">'.$bookingDate.'</td>
								<td width="75" align="center">'.$ConsignmentNo.'</td>
								<td width="100" align="center">'.$Destination.'</td>
								<td width="30" align="center">'.$Weight.'</td>
								<td width="40" align="center">'.$Amount.'</td>
								<td width="75" align="center">'.$insuChrg.'</td>
								<td width="65" align="center">'.$FnalAmount.'</td>                            							
							 </tr>';
							 
							$i=$i+1; }
		$html2=$html.$row.'	<tr>
									<td width="400" colspan="7" align="right" style="padding-right:10px">Total</td>
									<td width="65" align="center">'.sprintf('%0.2f',$sum).'/-</td>
								</tr>
								
								<tr>
									<td width="400" colspan="7" align="right" style="padding-right:10px">Fuel Charges ('.$cust[1]['Fuel_Surcharge'].'%)</td>
		  ...
		  $file_to_save = PATH_PDF."/invoice/".$cust[1]['Branch_Code']."-".$invoiceNo.".pdf";
		  file_put_contents($file_to_save,  $output);
			...
			$mail->Subject = "Your invoice (#".$cust[1]['Branch_Code']."-".$invoiceNo.")";
	 ...
	 $html='
		...
		$html2=$html.$row.'	<tr>
				  ...
				  <ul>
					...
					<li>By A/C payee Cheque/Draft favouring '.$cust[1]['Branch_Name'].'</li>
		  ...
		  file_put_contents($file_to_save,  $output);
			...
			$mail->From="mailer@courierbills.online";
			$mail->FromName="courierbills.online mailer";
			$mail->Sender="mailer@courierbills.online";
			$mail->AddReplyTo("replies@courierbills.online", "Replies for courierbills.online");
...
if($_POST['type']=="update")
{
	...
	try
	{
		...
		///*******************************************************
		/// Unlink the old PDF of Invoice ////////////////////////
		///*******************************************************
		$res=$rateClass->ExecuteQuery("SELECT Invoice_No FROM tbl_invoices WHERE Invoice_Id=".$_POST['invoiceId']."");
		$invoice=$res[1]['Invoice_No'];
		unlink(PATH_PDF."/invoice/".$invoice.".pdf");
...
abc;
		...
		';
		  ...
		  $dompdf->render();
		  $output = $dompdf->output();
		  $file_to_save = PATH_PDF."/invoice/".$invoiceInfo[1]['Invoice_No'].".pdf";
		  file_put_contents($file_to_save,  $output);
